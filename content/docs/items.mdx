...
and its item to exist. If the reference cannot be resolved (the provider plugin is
missing or the id is wrong), the voucher falls back to its `material` (default
`PAPER`). If a custom item shows up as paper, check the provider plugin is
installed and the id is correct.
</Callout>
