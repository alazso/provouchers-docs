...
Ids resolve at join time, so `/voucher reload` fixes a typo without a restart;
...
The [Stash](/docs/stash) of virtual vouchers players claim with `/stash`. Read at
startup; changing these needs a restart.
