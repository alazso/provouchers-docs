...
| `playtime` | `time` | Total playtime is at least this duration (`"2h"`, `"30m"`) | nothing |
| `item` | `material`, `amount` | The inventory holds at least this many of the material | nothing |
| `advancement` | `advancement` | The player completed this advancement key | nothing |
...
### Playtime, items, and advancements

```yaml
conditions:
  - type: playtime
    time: "10h"
    deny: "<red>Play 10 hours first."
  - type: item
    material: DIAMOND
    amount: 3
    deny: "<red>Bring 3 diamonds."
  - type: advancement
    advancement: "minecraft:story/mine_diamond"
    deny: "<red>Mine a diamond first."
```

`item` only checks the inventory; it never takes the items. `advancement` takes a
namespaced key; an unknown key denies rather than passing.
