...
ProVouchers submits anonymous, aggregate usage statistics through
[bStats](https://bstats.org) and FastStats. The data helps prioritise which
platforms, integrations, and features to invest in. It is enabled by default and
can be turned off with a single setting.
...
- **bStats** publishes a public page per plugin, with the charts above rendered as
  graphs.
- **FastStats** receives the same definitions.

Each chart is defined once and reported to both backends, and the single
`metrics.enabled` switch governs both.
