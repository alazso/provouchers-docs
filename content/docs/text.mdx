...
`%arg%`, `%random:min-max%`, and the named `%random:min-max:name%`) before MiniMessage.
These are always available, even without PlaceholderAPI, and resolve everywhere text is
rendered, including a voucher's display name and lore, not only rewards and messages.
`%arg%` only has a value when redeeming a code with an argument, so it is blank in
voucher names and lore. A voucher's display name and lore also accept `%expiry%`, which
renders its expiry as `in 30d`, `on 2026-12-31`, or `never`.
